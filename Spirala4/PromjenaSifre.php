...
<?php
	...
	<div class="Glavno">
		...
		<form name="FormaPromjenaSifre" method="get"  action="PromjenaSifre.php" >
			  Stari password :<br> <input type="password" name="StariPassword" > <br>
			  Novi Password :<br> <input type="password" name="NoviPassword" > <br>	  
			  Potvrdi password :<br> <input type="password" name="potvrdapw"> <br>
			  <input id="Dugme" class="Dugme" type="submit" name="PromjenaSifre" value ="Promijeni šifru">
		</form> 
	</div>
	<?php 

        if (isset($_REQUEST['PromjenaSifre']) && $_REQUEST['PromjenaSifre'] == 'Promijeni šifru') {

			$veza = new PDO("mysql:dbname=saputevibaza;host=localhost;charset=utf8", "sapuser", "sapuser");
			$veza->exec("set names utf8");
			$upit1=$veza->query("select id,password from users where username='".$_SESSION['username']."'");
			if (!$upit1) {
				          $greska = $veza->errorInfo();
				          print "SQL greška: " . $greska[2];
				          exit();
				     }
			$user=$upit1->fetch(PDO::FETCH_ASSOC);

	     	if($_REQUEST['NoviPassword']==$_REQUEST['potvrdapw'] && $user['password']==md5($_REQUEST['StariPassword']))
	     	{

				$upit2=$veza->query("update users set password='".md5($_REQUEST['NoviPassword'])."' where username='".$_SESSION['username']."'");
				if (!$upit2) {
				...
				echo '<script language="javascript">';
				echo 'alert("Potvrda passworda nije ista kao i password ili stari password nije korektan!")';
				echo '</script>';
			}
        }
